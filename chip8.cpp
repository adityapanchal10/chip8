...
void chip8::playSound(SDL_AudioDeviceID deviceId, uint32_t wavLength, uint8_t *wavBuffer)
{     
    if (sound_ > 0) {
        if(sound_ == 1) {
...
int chip8::loop()
{
    ...
    uint32_t wavLength;
    uint8_t *wavBuffer;
    SDL_LoadWAV("sound/beep.wav", &wavSpec, &wavBuffer, &wavLength);
    ...
    while (running_)
    {
        ...
        handleCounters();
        playSound(deviceId, wavLength, wavBuffer);
